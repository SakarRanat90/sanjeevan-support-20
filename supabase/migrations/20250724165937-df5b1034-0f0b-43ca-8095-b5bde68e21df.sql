...
-- Add input validation function for vitals (fixed RAISE syntax)
...
BEGIN
  ...
  IF NEW.heart_rate < 40 OR NEW.heart_rate > 220 THEN
    RAISE EXCEPTION 'Invalid heart rate: must be between 40-220 bpm';
  END IF;
  
  -- Validate temperature range (30-45°C)
  IF NEW.temperature < 30.0 OR NEW.temperature > 45.0 THEN
    RAISE EXCEPTION 'Invalid temperature: must be between 30-45°C';
  END IF;
  
  -- Validate SpO2 range (70-100%)
  IF NEW.spo2 < 70 OR NEW.spo2 > 100 THEN
    RAISE EXCEPTION 'Invalid SpO2: must be between 70-100%';
  END IF;
...
-- Add input sanitization function for text fields
CREATE OR REPLACE FUNCTION public.sanitize_text_input()
RETURNS TRIGGER AS $$
BEGIN
  -- Remove potentially dangerous characters and limit length
  IF TG_TABLE_NAME = 'profiles' THEN
    NEW.full_name := LEFT(TRIM(REGEXP_REPLACE(NEW.full_name, '[<>&"'']', '', 'g')), 100);
    NEW.address := LEFT(TRIM(REGEXP_REPLACE(NEW.address, '[<>&"'']', '', 'g')), 500);
    NEW.contact_number := REGEXP_REPLACE(NEW.contact_number, '[^0-9+\-\s()]', '', 'g');
    NEW.emergency_contact := REGEXP_REPLACE(NEW.emergency_contact, '[^0-9+\-\s()]', '', 'g');
  ELSIF TG_TABLE_NAME = 'medications' THEN
    NEW.medication_name := LEFT(TRIM(REGEXP_REPLACE(NEW.medication_name, '[<>&"'']', '', 'g')), 100);
    NEW.dosage := LEFT(TRIM(REGEXP_REPLACE(NEW.dosage, '[<>&"'']', '', 'g')), 100);
    NEW.frequency := LEFT(TRIM(REGEXP_REPLACE(NEW.frequency, '[<>&"'']', '', 'g')), 50);
  END IF;
...
CREATE TRIGGER sanitize_profiles_text
  BEFORE INSERT OR UPDATE ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.sanitize_text_input();

CREATE TRIGGER sanitize_medications_text
